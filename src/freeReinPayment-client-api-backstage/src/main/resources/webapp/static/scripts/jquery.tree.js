...
(function($) {
    ...
    $.fn.treeview = function(settings) {
        var dfop =
            {
                ...
                url: false, //异步请求的url
                cbiconpath: virtualPath+"/images/icons/", //checkbox icon的目录位置
                icons: ["checkbox_0.gif", "checkbox_1.gif", "checkbox_2.gif"],
                emptyiconpath: virtualPath+"/images/default/s.gif", //checkbxo三态的图片
        ...
        function asnyloadc(pnode, isAsync, callback) {
            if (dfop.url) {
                var param = {"token":tokenVal};
                if (pnode && pnode != null) {
                    //alert("111113333111111")
        ...
        function builparam(node) {
            ...
            return {"token":tokenVal,"id":encodeURIComponent(node.id)};
        }
        function bindevent() {
            $(this).hover(function() {
        ...
        function getckNoHalfCk(items, c, fn) {
            for (var i = 0, l = items.length; i < l; i++) {
                (items[i].showcheck == true && (items[i].checkstate == 1)) && c.push(fn(items[i]));
                if (items[i].childNodes != null && items[i].childNodes.length > 0) {
                    getckNoHalfCk(items[i].childNodes, c, fn);
                }
            }
        }

        function getckHalfCk(items, c, fn) {
            for (var i = 0, l = items.length; i < l; i++) {
                (items[i].showcheck == true && (items[i].checkstate == 2)) && c.push(fn(items[i]));
                if (items[i].childNodes != null && items[i].childNodes.length > 0) {
                    getckHalfCk(items[i].childNodes, c, fn);
                }
            }
        }
        ...
        me[0].t = {
            ...
            getSelectedCkAndHalfCkValues: function () {
                var s = [];
            ...
            getSelectedValues: function () {
                var s = [];
                getckNoHalfCk(treenodes, s, function (item) { return item.value; });
                return s;
            },
            getSelectedHalfCkValues: function () {
                var s = [];
                getckHalfCk(treenodes, s, function (item) { return item.value; });
                return s;
            },
    ...
    //获取所有选中的节点的Value数组(只有半选)
    $.fn.getTSVsByHalfCk = function () {
        if (this[0].t) {
            return this[0].t.getSelectedHalfCkValues();
        }
        return null;
    };

    //获取所有选中的节点的Value数组(包含半选)
    $.fn.getTSVsByCkAndHalfCk = function () {
        if (this[0].t) {
            return this[0].t.getSelectedCkAndHalfCkValues();
        }
        return null;
    };
    //获取所有选中的节点的Item数组
