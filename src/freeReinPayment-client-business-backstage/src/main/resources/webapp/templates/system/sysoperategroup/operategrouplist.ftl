...
<div class="main-content breadcrumbs ">
	<ul class="breadcrumb">
		...
		<li class="active">
			<span class="Current_page iframeurl" name="" style="color: rgb(51, 51, 51); cursor: default;">操作组管理</span>
		</li>
	</ul>
</div>
	...
	<div class="col-sm-12">
	   <div class="panel panel-default">
				...
				<form id="formSearch" class="form-horizontal" action="/list">
					<div class="search_style">
				      <ul class="search_content clearfix">
				       <li><label class="l_f">操作组名称</label><input id="cname" type="text"  class="text_add" placeholder="操作组名称"  style=" width:400px"/></li>
	    ...
	    <div id="toolbar" class="btn-group">
			<button id="system_groupAdd" type="button" class="btn btn-primary disabled">
				<span class="glyphicon-plus disabled" aria-hidden="true"></span>新增
			</button>
			<button id="system_groupEdit" type="button" class="btn btn-primary disabled">
				<span class="icon-pencil" aria-hidden="true"></span>修改
			</button>
			<button id="system_groupDel" type="button" class="btn btn-primary disabled">
...
function add(){
	var url = virtualPath+'/system/operategroup/create';
    openTwoWin('45%','70%',url);
}

function edit(){
	var rows = $("#tablelist").bootstrapTable('getSelections');
    if (rows.length==1){
        var url = virtualPath+'/system/operategroup/edit/'+rows[0].groupId;
        openTwoWin('45%','70%',url);
    }
    else{
        layer.alert("请选中一行数据", {});
...
function del(){
	...
	if (rows && rows.length > 0){
	    layer.confirm("确定删除吗?","信息",function () {
	        ...
	        var url = virtualPath+"/system/operategroup/delete";
